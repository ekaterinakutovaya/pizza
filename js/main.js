...
const app = () => {
    ...
    Array.from(addBtns).forEach((btn) => {
        btn.addEventListener('click', (e) => {
          ...
          if (!cart) {
            cart = [{ productId: +selectedProductId, count: 1 }];
        ...
        btn.addEventListener('click', function () {
            ...
            cartItems.forEach((item) => {
                if (item.dataset.id == this.dataset.delete) {
                    ...
                    const filteredProducts = products.filter((product) => product.productId != item.dataset.id)
                    localStorage.setItem('selectedProduct', JSON.stringify(filteredProducts))
                    item.remove()
                }
            });
        });
